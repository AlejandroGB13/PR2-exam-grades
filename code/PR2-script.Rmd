...
plot(data$gender,data$meangrade) 
```
...
plot(data$race.ethnicity, data$meangrade) 
```
...
plot(data$parental.level.of.education, data$meangrade, cex.axis = 0.5) 
```
...
plot(data$lunch, data$meangrade) 
```
...
plot(data$test.preparation.course, data$meangrade) 
```
...
Tras el análisis realizado, podemos concluir que no hay diferencia significativas en las notas entre mujeres y hombres. Pero sí hay diferencia es en relación a la raza y la educación de los padres, donde los grupos D y E sobresalen de los demás y los hijos con padres con estudios superiores también reciben mejores notas. Además, los estudiantes con el plan de comida estándar tiene mejores notas.

Por último, cabe destacar que los estudiantes tienen tendencia a sacar notas similares en las asignaturas muestreadas.
